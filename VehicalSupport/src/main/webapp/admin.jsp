...
<!--Header-->
    ...
    <section class="title">
        <div class="container">
 		...
 		<c:forEach items="${reqobj}" var="req">
 		...
 		<form action="selectassistant" method="post">
 		<input type="hidden" value="${req.service}" name="assitype">
 		<input type="hidden" value="${req.reqid}" name="reqid">
 		<button type="submit" style="margin-top:17px;">Select Assistance</button>
 		</form>
 		
 		<% 
 		model obj = new model();
 		String reqid = session.getAttribute("id").toString();
 		if (obj.search(reqid))  
 		{
 		%> 
 		...
 		<input type="hidden" value="${req.reqid}" name="reqid">
 		<button style="margin-top:-3px;" type="submit" >Accept Request</button>
 		</form><% } %>
 		</div>
 		</c:forEach>
